...
// ■■ KeyboardDevice ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ class KeyboardDevice : public IODevice {
...
public:
    ...
    {
        opened = true;
        std::cout << "[KB] " << name << " opened\n";
        return true;
    }
    void close() override
    {
        opened = false;
        std::cout << "[KB] " << name << " closed\n";
    }
    string read(size_t n) override
    {
        ...
        std::cout << "[KB] read " << out.size() << " bytes: '" << out << "'\n";
        return out;
    }
    bool write(const string &s) override
    {
        std::cout << "[KB] keyboards are input-only\n";
        return false;
    }
...
// ■■ DiskDevice ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ class DiskDevice : public IODevice {
...
{
    opened = true;
    std::cout << "[DISK] " << name << " spun up\n";
    return true;
}
void close() override
{
    opened = false;
    std::cout << "[DISK] " << name << " parked\n";
}
string read(size_t sector) override
{
    ...
    std::cout << "[DISK] read sector " << sector << "\n";
...
bool write(const string &data) override
{
    ...
    std::cout << "[DISK] wrote " << data.size() << " bytes to sector 0\n";
    return true;
}
string getInfo() const override
{
    ostringstream ss;
    ss << "DiskDevice '" << name << "' | sectors: " << totalSectors << " | capacity: " << totalSectors * SECTOR / 1024 << " KB";
    return ss.str();
}
}
;
// ■■ NetworkDevice ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ class NetworkDevice : public IODevice {
...
{
    opened = true;
    std::cout << "[NET] " << name << " socket opened\n";
    return true;
}
void close() override
{
    opened = connected = false;
    std::cout << "[NET] " << name << " disconnected\n";
}
string read(size_t n) override
{
    if (!opened)
...
bool write(const string &data) override
{
    if (!opened)
        return false;
    std::cout << "[NET] sent " << data.size() << " bytes to "
         << ip << ":" << port << "\n";
    return true;
}
string getInfo() const override
...
void add(shared_ptr<IODevice> dev)
{
    ...
    std::cout << "[REG] registered: " << dev->getInfo() << "\n";
}
